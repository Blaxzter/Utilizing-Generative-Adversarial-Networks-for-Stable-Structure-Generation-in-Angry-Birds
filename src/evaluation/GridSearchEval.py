...
class GridSearchEval:
    def __init__(self):
        ...
        self.level_visualizer = LevelVisualizer(line_size = 1, id_font_size = 3, dpi = 500)
        ...
        self.show_in_graph['total'] = False
                ...
                html.Div([
                    ...
                    dcc.Checklist(
                        ...
                        options = [
                            ...
                            {"label": 'Min Destroyed Blocks', "value": 'destroyed_blocks'}
                        ],
                        value = ['none_selected'],
                        inline = True
                    ),
        ...
        def update_line_chart(bool_parameters, optimized_value, negative_air_value, cutoff_point, level_selection, biggest_difference,
            ...
            if 'Compare Values' in compare_values:
                ...
                for value in self.parameter_dict[compare_selection]['values']:
                    ...
                    data.append(
                        go.Bar(name = f'Value: {value}', x = collected_data_labels, y = y_data, text = np.round(y_data, decimals = 2), textfont = dict(size = 18))
                    )

                self.current_figure = go.Figure(data = data)
                self.current_figure.update_layout(barmode = 'group')
                self.current_figure.update_layout(legend = dict(
                    orientation = "h",
                    yanchor = "bottom",
                    y = 1.02,
                    xanchor = "right",
                ...
                if 'none_selected' not in optimized_value:
                    ...
                    for c_optimized_value in optimized_value:
                        ...
                        label_translate = dict(
                            ...
                            destroyed_blocks = 'Min Destroyed Blocks'
                        )

                        data.append(
                            go.Bar(name = label_translate[c_optimized_value], x = collected_data_labels, y = y_data, text = np.round(y_data, decimals = 2), textfont = dict(size = 18))
                        )
                    self.current_figure = go.Figure(data = data)
                    self.current_figure.update_layout(barmode = 'group')
                    self.current_figure.update_layout(legend = dict(
                    ...
                    data.append(
                        go.Bar(x = collected_data_labels, y = y_data, text = np.round(y_data, decimals = 2), textfont = dict(size = 18))
                    )
                    ret_figure = go.Figure(data = data)
                    ret_figure.update_layout(legend = dict(
                        orientation = "h",
                        yanchor = "bottom",
                        y = 1.02,
                        xanchor = "right",
                    ...
                    self.current_figure = ret_figure
